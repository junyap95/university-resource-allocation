...
export default function DateBasedAllocator({ bookingData, setAllocatedData, setHighlightedDate }) {
    ...
    const handleChangeAlgo = useCallback((event) => {
        setDateAndAlgo((prev) => ({
            ...prev,
    ...
    return (
        <div style={{ display: "flex", alignItems: "center" }}>
            ...
            <Box sx={{ minWidth: 200 }}>
                <FormControl fullWidth>
                    <InputLabel id="demo-simple-select-label">Greedy Algorithm</InputLabel>
                    <Select
                        labelId="demo-simple-select-label"
                        id="demo-simple-select"
                    ...
                    >
                        <MenuItem key={"startTime"} value={"timeGreedy"}>Earliest Start Time</MenuItem>
                        <MenuItem key={"duration"} value={"durationGreedy"}>Longest Duration</MenuItem>
                        <MenuItem key={"random"} value={"randomGreedy"}>Random</MenuItem>
                    </Select>
                </FormControl>
            </Box>
